  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select
        ...
        filterable
        clearable
        :placeholder="$t('table.rateLimitRule.requestMethod')"
        value=""
      ...
      <el-dropdown v-has-any-permission="['gatewayRouteLimitRule:add','gatewayRouteLimitRule:delete']" trigger="click" class="filter-item">
        <el-button>
          {{ $t('table.more') }}<i class="el-icon-arrow-down el-icon--right" />
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-has-permission="['gatewayRouteLimitRule:add']" @click.native="add">{{ $t('table.add') }}</el-dropdown-item>
          <el-dropdown-item v-has-permission="['gatewayRouteLimitRule:delete']" @click.native="batchDelete">{{ $t('table.delete') }}</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column :label="$t('table.operation')" align="center" min-width="60px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <i v-has-permission="['gatewayRouteLimitRule:update']" class="el-icon-setting table-operation" style="color: #2db7f5;" @click="edit(row)" />
          <i v-has-permission="['gatewayRouteLimitRule:delete']" class="el-icon-delete table-operation" style="color: #f50;" @click="singleDelete(row)" />
        </template>
      </el-table-column>
    </el-table>
...
import RouteLimitRuleEdit from './Edit'

export default {
  name: 'RouteLimitRule',
  components: { Pagination, RouteLimitRuleEdit },
  filters: {
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.$refs.edit.setRouteLimitRule(row)
    ...
    batchDelete() {
      ...
      this.$confirm(this.$t('tips.confirmDelete'), this.$t('common.tips'), {
        ...
        this.selection.forEach((c) => {
          rateLimitRule.push(c.id)
        })
        this.delete(rateLimitRule)
      }).catch(() => {
    ...
    fetch(params = {}) {
      ...
      this.loading = true
      this.$get('system/gatewayRouteLimitRule/list', {
        ...params
      }).then((r) => {
