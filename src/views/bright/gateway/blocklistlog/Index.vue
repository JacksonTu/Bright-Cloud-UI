<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-dropdown v-has-any-permission="['gatewayBlockListLog:delete']" trigger="click" class="filter-item">
        <el-button>
          {{ $t('table.more') }}<i class="el-icon-arrow-down el-icon--right" />
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-has-permission="['gatewayBlockListLog:delete']" @click.native="batchDelete">{{ $t('table.delete') }}</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column :label="$t('table.operation')" align="center" min-width="60px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <i v-has-permission="['gatewayBlockListLog:delete']" class="el-icon-delete table-operation" style="color: #f50;" @click="singleDelete(row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    batchDelete() {
      ...
      this.$confirm(this.$t('tips.confirmDelete'), this.$t('common.tips'), {
        ...
        this.selection.forEach((c) => {
          blockListLogs.push(c.id)
        })
        this.delete(blockListLogs)
      }).catch(() => {
    ...
    fetch(params = {}) {
      ...
      this.loading = true
      this.$get('system/gatewayBlockListLog/list', {
        ...params
      }).then((r) => {
