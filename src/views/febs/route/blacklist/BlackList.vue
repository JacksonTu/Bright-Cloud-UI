...
export default {
  ...
  methods: {
    ...
    batchDelete() {
      ...
      this.$confirm(this.$t('tips.confirmDelete'), this.$t('common.tips'), {
        ...
        this.selection.forEach((r) => {
          blockListIds.push(r.id)
        })
        this.delete(blockListIds)
      }).catch(() => {
    ...
    delete(blockListIds) {
      this.loading = true
      r.delete('route/auth/blockList', { ids: blockListIds }).then(() => {
        this.$message({
    ...
    fetch(params = {}) {
      ...
      this.loading = true
      axios.all([
        r.get('route/auth/blockList/data', { ...params }),
        r.get('route/auth/blockList/count', { ...params })
      ]).then(v => {
        this.total = v[1].data
        this.list = v[0].data
        this.loading = false
