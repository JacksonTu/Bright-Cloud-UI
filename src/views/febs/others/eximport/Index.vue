...
export default {
  ...
  data() {
    return {
      uploadAction: `${process.env.VUE_APP_BASE_API}test/eximport/import`,
      dialogVisible: false,
      tableKey: 0,
      loading: false,
      list: null,
  ...
  methods: {
    ...
    templateDownload() {
      this.$download('test/eximport/template', {}, 'excel_import_template.xlsx')
    },
    exportExcel() {
      this.$download('test/eximport/excel', {
    ...
    fetch(params = {}) {
      ...
      this.$get('test/eximport', { ...params }).then((r) => {
